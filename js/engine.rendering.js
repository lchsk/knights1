var drawHealthBox  = function(unit,x,y) {
	var fill = Engine.ctx.fillStyle;
	var stroke = Engine.ctx.strokeStyle;
	var lenght = unit.health/6.25;  //  100/16 = 6.25
	
	Engine.ctx.beginPath();
    Engine.ctx.rect(x + lenght, y +1, 16-lenght, 3);
	...
	Engine.ctx.beginPath();
    Engine.ctx.rect(x , y +1, lenght, 3);
    Engine.ctx.fillStyle = 'ForestGreen';
    Engine.ctx.fill();
    Engine.ctx.strokeStyle = 'ForestGreen';
    Engine.ctx.stroke();
	
	Engine.ctx.beginPath();
    Engine.ctx.rect(x-1 , y , 18, 5);
            ...
            if (units[i] && units[i].unit_class.image && units[i].DrawUnit())
            {
                ...
                */
                ...
                if ($.inArray(units[i], selected_units) > -1)
                {
                    ...
                    * Drawing life strip
                    */
					drawHealthBox(units[i],units[i].x - View.x,units[i].y - View.y);
                 /*   if (units[i].health > 80)
                        Engine.ctx.drawImage(GameSprites.life_strip[100], units[i].x - View.x, units[i].y - View.y+8); 
                    else if (units[i].health > 60)
                        Engine.ctx.drawImage(GameSprites.life_strip[80], units[i].x- View.x, units[i].y - View.y+8);     
                    else if (units[i].health > 40)
                        Engine.ctx.drawImage(GameSprites.life_strip[60], units[i].x - View.x, units[i].y - View.y+8); 
                    else if (units[i].health > 20)
                        Engine.ctx.drawImage(GameSprites.life_strip[40], units[i].x - View.x, units[i].y - View.y+8); 
                    else
                        Engine.ctx.drawImage(GameSprites.life_strip[20], units[i].x - View.x, units[i].y - View.y+8);   */
                }
...
*/
    ...
    */
    ...
    if (selected_units.length == 1 && selected_units[0].what == 'building')
    {
		...
		drawHealthBox(selected_units[0],buttons['building_health'].x + 12, buttons['building_health'].y + 16);
