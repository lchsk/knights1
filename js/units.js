...
// Worker
unit_skeletons_worker = new UnitClass(GameSide.SKELETONS, "Worker", false, 0.9, UnitType.WORKER, 30, 0, 'sworker',0);
...
// Warrior
unit_skeletons_warrior = new UnitClass(GameSide.SKELETONS, "Warrior", true, 0.8, UnitType.KNIGHT, 40, 20,'swarrior',1);
...
// Archer
unit_skeletons_archer = new UnitClass(GameSide.SKELETONS, "Archer", true, 0.7, UnitType.ARCHER, 70, 40, 'sarcher',3);
...
// Knight
unit_skeletons_knight = new UnitClass(GameSide.SKELETONS, "Knight", true, 0.7, UnitType.KNIGHT, 70, 40,'sknight',1);
...
// Monk
unit_skeletons_monk = new UnitClass(GameSide.SKELETONS, "Wizard", true, 0.7, UnitType.SPECIAL, 90, 40,'smonk',1);
...
// Worker
unit_knights_worker = new UnitClass(GameSide.KNIGHTS, "Worker", false, 0.8, UnitType.WORKER, 30, 'kworker',0);
...
// Warrior
unit_knights_warrior = new UnitClass(GameSide.KNIGHTS, "Warrior", true, 0.8, UnitType.WARRIOR, 40, 20, 'kwarrior',1);
...
// Archer
unit_knights_archer = new UnitClass(GameSide.KNIGHTS, "Archer", true, 0.9, UnitType.ARCHER, 30, 60, 'karcher',3);
...
//Knight
unit_knights_knight = new UnitClass(GameSide.KNIGHTS, "Knight", true, 0.7, UnitType.KNIGHT, 60, 60, 'kknight',1);
...
// Paladin
unit_knights_paladin = new UnitClass(GameSide.KNIGHTS, "Paladin", true, 0.5, UnitType.SPECIAL, 100, 90, 'kpaladin',1);
...
var Unit = function(unit_class){
    ...
    */
    this.fight = function(ms){
        ...
        if (this.fighting)
        {
            ...
            {
                that.current_frame++;
                _ms -= (animation_time.attack/100);
				
				if(that.current_frame == Math.round((frames-start_frame)/2))
				{
					this.can_offense++;
				}
				else if (this.current_frame == frames)
				{
	...
	this.offense = function(){
		...
		{
			this.can_offense -= 1;
			return that.unit_class.GetAttack();
		}
		else
		{
			return 0;
    ...
    */
    this.die = function(ms){
		...
		else if( !this.IsMoving() && !this.fighting ){
			_ms = 0;
		}
    ...
    */
    this.move = function(ms){
        ...
        if(this.IsMoving() && ! this.dying)
        {
			...
			var road_on_this_step = this.unit_class.GetSpeed() * ms*50;

            switch (dir)
            {
                case Direction.N:
                    //this.y -= this.unit_class.GetSpeed() * ms;
                    this.y -= road_on_this_step;
                    break;

                case Direction.E:
                    //this.x += this.unit_class.GetSpeed() * ms;
                    this.x += road_on_this_step;
                    break;
            ...
            this.road_travelled += road_on_this_step;
           ...
           // if ((Calc.is_within(this.road_travelled, 31, 33, true))) {
			...
			//				console.log('---------------------');
                ...
                this.road_travelled -= config.tile_width;
                this.road.shift();
				...
				if(this.road.length > 0)
				{
					...
					if(current_map.build_map[this.road[0]] != 0)
					{
            ...
            */
            if (dir > -1)
            {
                ...
                if (_ms > animation_time.walkcycle/100)// /10
                {
                    this.current_frame++;
					//_ms = 0;
                    _ms -= animation_time.walkcycle/100;// /10
			    }
            }
