...
var View = View || (function(){
        return {
            ...
            move : function(){
				...
				if(this.y  <0)
					this.y = 0;
				else if( this.y > config.view_max_y)
					this.y = config.view_max_y;
...
function handle_mouse_move(event)
{
	...
	if(pos.x >=0  && pos.x <= config.view_width &&
		pos.y >=0  && pos.y <= config.view_width ){
...
{    
	if (event.button == config.inverse_button)
    {
    ...
    if (event.button == 2-config.inverse_button)
    {
